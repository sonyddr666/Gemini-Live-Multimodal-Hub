...
import { Settings, Mic, MicOff, Send, MessageSquare, AudioLines, Sparkles, Search, Code, Cpu, Edit2, Trash2, Plus, Eye, Download } from 'lucide-react';
...
export default function App() {
  ...
  // Save active instruction to localStorage
  useEffect(() => {
    ...
    if (apiKey) {
      ...
      sessionManagerRef.current.setCallbacks(
        (msg) => {
          setMessages((prev) => {
            ...
            if (prev.length === 0) {
              ...
              // If it's a tool call or system message, always add as new
              newMessages = [...prev, msg];
            } else {
              // Find the last message from the same role and same thinking state
              // We only look back until we hit a different role (e.g., user)
              let targetIndex = -1;
              for (let i = prev.length - 1; i >= 0; i--) {
                if (prev[i].role !== msg.role) {
                  break; // Stop if we hit a different role
                }
        ...
        (newStatus) => {
          ...
          if (newStatus === 'Disconnected' || newStatus === 'Connection Failed') setIsConnected(false);
        }
      );
    }
    return () => {
    ...
    <div className="flex h-screen bg-[#1e1e1e] text-gray-200 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        <div className="p-4 bg-[#1e1e1e] border-t border-white/10">
          <form onSubmit={handleSendText} className="flex gap-2 max-w-4xl mx-auto">
            <button
              type="button"
          ...
          <CollapsibleSection title="🕓 HISTÓRICO" defaultOpen>
            <div className="space-y-3">
              <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
                {history.length === 0 ? (
                  <p className="empty-state">Nenhuma sessão salva.</p>
                ) : (
                  history.map(session => (
              ...
              <button 
                onClick={() => {
                  setInstructions(prev => {
                    ...
                    if (exists) {
                      return prev.map(i => i.id === editingInstruction.id ? editingInstruction : i);
                    }
